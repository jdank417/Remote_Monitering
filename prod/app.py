...
MACHINES_FILE = os.path.join(RESOURCES_DIR, "machines.json")


def show_notification(title, message):
    """
    Display a system notification on macOS using AppleScript.
    """
    try:
...
class SSHWorker(QThread):
    ...
    def run(self):
        ...
        # If a timeout occurred, show a system notification and emit a result.
        if ssh_client == "timeout":
            show_notification("VPN Connection",
                              f"Could not reach {host} in 10 seconds.\nAre you sure your VPN is enabled?")
...
class SSHApp(QMainWindow):
    ...
    def load_machines(self):
        """Load machine data from the resources/machines.json file."""
        try:
    ...
    def save_machines(self):
        """Save machine data to the resources/machines.json file."""
        try:
            os.makedirs(RESOURCES_DIR, exist_ok=True)
            with open(MACHINES_FILE, "w") as file:
                json.dump(self.machines, file)
    ...
    def update_grid(self):
        ...
        for i in range(len(self.machines)):
            ...
            if host not in self.machine_labels:
    ...
    def edit_machines_file(self):
        ...
        try:
            subprocess.call(["open", MACHINES_FILE])  # macOS-specific; change if needed for other OS
        except Exception as e:
